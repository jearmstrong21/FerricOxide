...
public class StatementParser {
    ...
    private static Block attemptBlock(TokenScanner scanner) {
        if (scanner.peek() instanceof GroupToken group && group.type == GroupToken.Type.CURLY_BRACKET) {
            ...
            return new Block(scanner.lastConsumedSpan(), statements);
        }
        return null;
    }
    ...
    private static Statement attemptIf(TokenScanner scanner) {
        if (scanner.peek() instanceof IdentToken ident && ident.value.equals("if")) {
            ...
            Expression cond = ExpressionParser.forceExpr(condScanner);
            ...
            if (scanner.peek() instanceof IdentToken elseIdent && elseIdent.value.equals("else")) {
                ...
                return new IfStmt(span, cond, then, otherwise);
            }
            return new IfStmt(span, cond, then, null);
        }
        return null;
    }
    ...
    private static Statement attemptReturn(TokenScanner scanner) {
        if (scanner.peek() instanceof IdentToken ident && ident.value.equals("return")) {
            ...
            Expression expr = ExpressionParser.forceExpr(scanner);
    ...
    private static Statement attemptWhile(TokenScanner scanner) {
        if (scanner.peek() instanceof IdentToken ident && ident.value.equals("while")) {
            ...
            return new WhileStmt(Span.concat(start, end), cond, body);
        }
        return null;
    }
    ...
    private static Statement attemptDeclareVar(TokenScanner scanner, boolean requireSemicolon) {
        if (scanner.peek() instanceof IdentToken ident && ident.value.equals("let")) {
            ...
            return new DeclareVar(Span.concat(start, end), type, new Identifier(nameSpan, name), value);
        }
        return null;
    }

    private static Statement attemptCall(TokenScanner scanner, boolean requireSemicolon) {
        Expression expr = ExpressionParser.forceExpr(scanner);
        Span end = maybeOptionalSemicolon(scanner, requireSemicolon);
        Span span = Span.concat(expr.span, end);
        if (expr instanceof CallExpr callExpr) return new CallStmt(span, callExpr);
        throw new SourceCodeException(span, "expected call expr");
    }

    private static Statement attemptAssign(TokenScanner scanner, boolean requireSemicolon) {
        ...
        if (attempt.peek() instanceof PunctToken punct && punct.type == PunctToken.Type.EQ) {
            ...
            Expression right = ExpressionParser.forceExpr(scanner);
            Span end = maybeOptionalSemicolon(scanner, requireSemicolon);
            return new Assign(Span.concat(left.span, end), left, right, BinaryOperator.DISCARD_FIRST);
        }
        return null;
    }
    ...
    private static Statement attemptFor(TokenScanner scanner) {
        if (scanner.peek() instanceof IdentToken ident && ident.value.equals("for")) {
            ...
            Statement init = forceStatement(forScanner, true);
            Expression condition = ExpressionParser.forceExpr(forScanner);
            forScanner.next().requirePunct(PunctToken.Type.SEMICOLON);
            Statement update = forceStatement(forScanner, false);
            forScanner.requireEmpty();
            Block block = forceBlock(scanner);
            return new ForStmt(Span.concat(start, block.span), init, condition, update, block);
        }
        return null;
    }

    public static Statement forceStatement(TokenScanner scanner, boolean requireSemicolon) {
        Statement stmt;
